...
class ArenaMatcher(object):
    ...
    def query_ranking(self, data, arena):
        """查询自己的排名
        """
        defer = Deferred()
        defer.addCallback(self._get_arena_rank, data, arena)
        defer.addCallback(self._check)
        defer.callback(True)
        return defer


    def match(self, data, arena):
        """匹配对手
        """
        defer = Deferred()
        defer.addCallback(self._get_arena_rank, data, arena)
        defer.addCallback(self._match_arena, data, arena)
        defer.addCallback(self._check)
        defer.callback(True)
        return defer


    def query_arena_users_by_ranking(self, data, arena, count, users):
        """按照排名查询演武场，获取对应用户信息
        args:
            users(out: 元组（userid, name, level, icon_id, title_level, score, ranking_index）)
        """
        defer = Deferred()
        defer.addCallback(self._get_arena_rank, data, arena)  #先查自己的排名
        defer.addCallback(self._query_arenas_by_ranking, arena, count, users)
        defer.addCallback(self._check)
        defer.callback(True)
        return defer
    ...
    def _calc_arena_rank(self, proxy, data, first_user_id):
        self.rank_base = proxy.get_ranking("arena", "score", first_user_id)
        self_rank = proxy.get_ranking("arena", "score", data.id)
        self.rank = self_rank - self.rank_base + 1
        logger.debug("arena self rank=%d, rank_base=%d" % (self.rank, self.rank_base))
        return True


    def _match_arena(self, status, data, arena):
        """匹配副本对手
        """
        assert status is True
        ...
        for rank in players_rank:
            if rank > self.users_count:  #超过本房间人数则跳过
                continue
            cache_proxy.search_by_rank("arena", "score", self.rank_base + rank - 1, self.rank_base + rank - 1)
        defer = cache_proxy.execute()
        defer.addCallback(self._select_players_info, data, arena, players_rank)
        return defer
